...
function ENSRegistrationComponent() {
  ...
  {
    const signer = context.library.getSigner()
    var domainNameHash = ethers.utils.namehash(ensDomainName)
    ...
    var minregistrationtime = ethController.MIN_REGISTRATION_DURATION()
    await new Promise(resolve => setTimeout(resolve, minCommitTime*1000))
    console.log('Waited ' + minCommitTime + ' seconds')
    txid = ethController.register(domainName, signer._address, ethers.utils.bigNumberify(31535999), commitSecret, {value:rentPrice * 1.05})
//    txid = ethController.register(domainName, signer._address, ethController.MIN_REGISTRATION_DURATION(), ethers.utils.formatBytes32String(domainName))
